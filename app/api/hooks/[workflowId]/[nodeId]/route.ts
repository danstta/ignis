...
import { getWorkflowById } from "@/lib/workflows/service";
...
export async function POST(
  ...
  let workflow;
  try {
    // By-id on purpose: this endpoint is public (no session, no workspace) and
    // the unguessable workflow uuid IS the capability. Tenancy flows from the
    // loaded row into the run the engine starts.
  ...
  if (workflow.active) {
    // Gate BEFORE enqueueing: the sender gets a definitive 402 instead of a 202
    // for a run that would silently never start. Sample capture above already
    // happened, so the event is still inspectable in the editor.
    try {
      await assertRunQuota(workflow.workspaceId);
    } catch (err) {
      if (err instanceof EntitlementLimitError) {
        console.log(`[hook ${workflowId}/${nodeId}] rejected: ${err.message}`);
