...
export async function GET(
  ...
  try {
    ...
    // The HMAC-verified state carries the workspace resolved at /start — every
    // write below is scoped to it, so tokens can't land in a foreign row.
    let id = state.connectionId;
    if (id && (await getConnection(state.workspaceId, id))) {
      await mergeConnectionConfig(state.workspaceId, id, tokens);
    } else {
      const conn = await createConnection(state.workspaceId, {
        type: provider,
        name: def.name,
        config: tokens,
      });
