...
import { getRunLogs, getRunScoped } from "@/lib/workflows/runs-service";
...
export default async function RunDetailPage({
  ...
  const run = await getRunScoped(ctx.workspace.id, runId);
  if (!run || run.workflowId !== id) notFound();
  const workflow = await getWorkflow(ctx.workspace.id, id);
