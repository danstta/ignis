...
function StepCard({
  ...
  return (
    <div className="relative">
      ...
      {/* Per-node toolbar shown next to the selected step (would overflow on phones). */}
      {selected ? (
        <div className="absolute left-full top-1/2 ml-2 hidden -translate-y-1/2 items-center gap-0.5 rounded-md border bg-background p-0.5 shadow-sm sm:flex">
...
const STEPS = [
  ...
  { step: "S2", label: "List Drive Images" },
  { step: "S3", label: "Rank Images" },
  { step: "S4", label: "LLM Prompt" },
...
export function WorkflowMockup() {
  return (
    <AppFrame path="/workflows/blog-to-social-posts">
      <div className="flex items-stretch">
        {/* Node palette — node-palette.tsx (full catalog, no inner scroll) */}
        ...
        <aside className="hidden w-[26rem] shrink-0 flex-col border-l bg-background lg:flex">
          <div className="shrink-0 border-b bg-background/95 p-3">
            <div className="flex items-center gap-2">
              <Input
                readOnly
                aria-label="Workflow name"
                value="Blog to Social Posts"
          ...
          <div className="flex-1">
            <div className="flex flex-col gap-4 p-4">
              ...
              <div className="flex flex-col gap-3">
                ...
                <BindingRow name="title" kind="text" token="{{S1.body.title}}" />
                <BindingRow name="subtitle" kind="text" token="{{S4.text}}" />
                <BindingRow name="background" kind="image" token="{{S3.best.url}}" />
              </div>
