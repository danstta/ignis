...
/** Auth signing secret (better-auth). Falls back to a dev-only constant outside production. */
...
/**
 * Master switch for new-account registration. Defaults to ON so a fresh
 * self-hosted clone works with zero config; set `SIGNUPS_ENABLED=false` to run
 * waitlist-only (existing users keep signing in, the sign-up page points at
 * the landing-page waitlist, and better-auth rejects registration server-side).
 */
export function signupsEnabled(): boolean {
  const v = process.env.SIGNUPS_ENABLED?.trim().toLowerCase();
  return v !== "false" && v !== "0";
}
 ...
 * Used to build webhook URLs that third-party services must reach. When unset,
...
/** Optional Google Maps Platform key used by Find Location Images for Places photos. */
