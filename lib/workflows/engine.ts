...
import { getWorkflowById } from "./service";
import {
  appendRunLog,
  createRun,
  getRun,
  ...
  const runNodeWork = async (
    ...
    try {
      ...
      const ctx: NodeRunContext = {
        nodeId: node.id,
        // Derived from the workflow row (never from a session or event payload):
        // nodes scope their resource lookups and storage keys with it.
        workspaceId,
        config,
...
export async function startRun(
  ...
  // By-id load: the engine has no session; tenancy derives from the row itself.
...
export async function resumeRun(
  ...
  const workflow = await step("resume:load-workflow", () =>
    getWorkflowById(run.workflowId),
  );
  if (!workflow) throw new Error("Workflow not found");
