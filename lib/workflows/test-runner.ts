...
async function runNode(
  ...
  try {
    ...
    const ctx: NodeRunContext = {
      nodeId: node.id,
      // The tester's own workspace: the graph arrives from the client, so any
      // resource ids inside it only resolve if they belong to this workspace.
      workspaceId,
      config,
...
export async function runWorkflowTest({
  ...
  async function executeNode(node: WorkflowNode): Promise<"continue" | "stop"> {
    const result = await runNode(graph, node, state, trigger, runId, workspaceId);
