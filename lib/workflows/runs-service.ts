...
/**
 * INTERNAL by-id load for the engine and Inngest handlers (no session there;
 * step ids must stay stable across replays). Session-facing callers must use
 * {@link getRunScoped} so a guessed run id can't cross workspaces.
 */
...
/**
 * A run only if its parent workflow belongs to `workspaceId` — the ownership
 * proof for every session-facing run read/control (status, stop, resume,
 * run-detail page). Tenancy lives on workflows; runs derive it via this join.
 */
export async function getRunScoped(workspaceId: string, id: string) {
  const rows = await db()
    .select({ run: workflowRuns })
    .from(workflowRuns)
    .innerJoin(workflows, eq(workflowRuns.workflowId, workflows.id))
    .where(
      and(eq(workflowRuns.id, id), eq(workflows.workspaceId, workspaceId)),
    )
    .limit(1);
...
export async function listRunsWithWorkflow(
  ...
  const conditions: SQL[] = [eq(workflows.workspaceId, workspaceId)];
