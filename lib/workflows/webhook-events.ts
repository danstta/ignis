...
import { webhookEvents, workflows } from "@/lib/db/schema";
...
/**
 * Most recent payload received by a Webhook node, or null. Scoped to the
 * caller's workspace via the parent workflow — captured payloads are
 * third-party data, so a foreign workflow id must resolve to nothing.
 */
export async function getLatestWebhookEvent(
  ...
  const rows = await db()
    .select({ event: webhookEvents })
    .from(webhookEvents)
    .innerJoin(workflows, eq(webhookEvents.workflowId, workflows.id))
    .where(
      and(
        eq(webhookEvents.workflowId, workflowId),
        eq(webhookEvents.nodeId, nodeId),
