...
export interface NodeRunContext<C = Record<string, unknown>> {
  ...
  nodeId: string;
  /**
   * The workspace the parent workflow belongs to. Nodes MUST pass this into
   * every resource lookup (getConnection/getTemplate/...) so ids embedded in a
   * workflow graph can never resolve another workspace's rows; storage writes
   * prefix their keys with it.
   */
  workspaceId: string;
