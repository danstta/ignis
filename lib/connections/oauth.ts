...
import { getConnectionById, mergeConnectionConfigById } from "./service";
...
/**
 * `workspaceId` is resolved from the verified session at /start and trusted by
 * the callback (the HMAC proves it wasn't altered), so tokens always land in a
 * connection row of the workspace that initiated the consent flow.
 */
...
export async function ensureFreshToken(connectionId: string): Promise<string> {
  // By-id on purpose: callers hold an already-authorized connection (an engine
  // node that resolved it through the workspace-scoped getConnection, or the
  // HMAC-verified public image proxy) — see getConnectionById.
  ...
  await mergeConnectionConfigById(connectionId, patch);
