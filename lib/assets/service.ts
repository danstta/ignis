...
import { assertStorageQuota } from "@/lib/entitlements";
...
export async function listAssets(workspaceId: string): Promise<Asset[]> {
  const rows = await db()
    .select()
    .from(assets)
    .where(and(eq(assets.workspaceId, workspaceId), eq(assets.kind, "upload")))
 ...
 */
export async function createAssetFromBytes(
  workspaceId: string,
  input: {
    name: string;
    contentType: string;
  ...
  await assertStorageQuota(workspaceId, input.data.byteLength);
  const ext = extensionForType(input.contentType);
  const storageKey = `ws/${workspaceId}/assets/${crypto.randomUUID()}.${ext}`;
...
export async function importSvgAsset(
  workspaceId: string,
  input: {
    name?: string;
    code: string;
  ...
  return createAssetFromBytes(workspaceId, {
    name,
