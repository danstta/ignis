...
- Nodes are organized into plugins (see `plugins/README.md` for how to add your own)
...
### Authentication
- Email/password accounts via [better-auth](https://www.better-auth.com/) — no external auth service required
- Optional Google/GitHub social login, enabled by setting `AUTH_GOOGLE_*` / `AUTH_GITHUB_*` OAuth env vars
...
- An [Inngest](https://www.inngest.com/) account for production (free tier available)
...
# Edit .env.local — DATABASE_URL and SESSION_SECRET are the only required values
...
To execute workflows locally, start the Inngest dev server in a separate terminal:
...
4. Run database migrations after the first deploy (`bun run db:migrate` against your production `DATABASE_URL`)

### Environment variables

Only `DATABASE_URL` and `SESSION_SECRET` are required to run the core app — after `db:migrate`, sign up for the first account at `/signup`. Everything else powers a specific integration and can be added later. See `.env.example` for the full annotated list.

```env
# Database
DATABASE_URL=postgresql://user:password@host:5432/dbname
# Direct (non-pooled) connection used by drizzle-kit migrations.
# Falls back to DATABASE_URL when unset — required on transaction poolers
# (Supabase port 6543, Neon pooled) where DDL must not run pooled.
DIRECT_URL=postgresql://user:password@host:5432/dbname

# Auth — secret used to sign sessions and hash tokens
SESSION_SECRET=a-long-random-secret-string
# Optional: set to false to close registration (waitlist-only mode) —
# existing users still sign in, /signup points at the landing-page waitlist
...
# Google OAuth (for the Google Drive connection)
GOOGLE_CLIENT_ID=
GOOGLE_CLIENT_SECRET=

# OpenAI (for vision-based nodes like Rank Images)
OPENAI_API_KEY=

# Pexels (optional, for polished Find Location Images results)
PEXELS_API_KEY=

# Google Maps Platform (optional, for exact Google Places photos)
GOOGLE_MAPS_API_KEY=
```

If you enable `CONNECTIONS_ENCRYPTION_KEY` on an existing deployment, run `bun scripts/encrypt-connections.ts` once to seal rows stored before encryption — and rotate any credentials that were stored unencrypted if the database or its backups may have been exposed.

### Find Location Images setup

The Find Location Images node uses OpenStreetMap Nominatim to geocode the location query, then searches image providers. Pexels is best for polished poster-style destination photos and requires `PEXELS_API_KEY`. Wikimedia Commons gives the most precise nearby geotagged search. Openverse adds a broader open-licensed catalog, but quality varies. None of these require a Google Maps key.

In the workflow editor, capture a webhook sample first, select only the fields you want to expose downstream, then insert those fields into the node's Location query — for example a city, venue name, or street address rather than the entire webhook body. Returned candidates include source, author, license, and attribution URLs when the provider returns them.

### Database migrations

Run migrations whenever you pull new changes:
...
This project is in early development and the scope is intentionally narrow right now. If you have ideas, open an issue first before submitting a large pull request. See [CONTRIBUTING.md](CONTRIBUTING.md) for dev setup and the quality bar.
