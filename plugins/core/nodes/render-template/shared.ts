...
export async function renderTemplateToStorage(
  ...
  for (const png of pngs) {
    // Tenancy rides on the template row: callers resolved it through the
    // workspace-scoped getTemplate, so its workspaceId is already proven.
    const key = `ws/${template.workspaceId}/renders/${crypto.randomUUID()}.png`;
