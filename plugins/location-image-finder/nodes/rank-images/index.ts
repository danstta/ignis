...
import {
  ...
  type PreparedImage,
  type ResponseFormat,
} from "@/lib/nodes/vision-image-utils";
import { normalizeImageCandidates } from "@/lib/nodes/image-input";
import type { ImageCandidate, NodeDefinition, NodeStepRunner } from "@/lib/nodes/types";
...
export const rankImagesNode: NodeDefinition<RankImagesConfig> = {
  ...
  async run(ctx) {
    ...
    const connection = await getConnection(ctx.workspaceId, ctx.config.connectionId);
